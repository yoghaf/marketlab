...
class SignalForwardReturnLog(Base):
    ...
    observation_epoch: Mapped[str | None] = mapped_column(String(32), index=True)
    observation_start_utc: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    observation_marker: Mapped[bool | None] = mapped_column(Boolean)
    evidence: Mapped[dict | None] = mapped_column(JSON)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
