...
from app.services.feature_context_join import FeatureContextJoinService  # noqa: E402
...
LOCK_PATH = ROOT / "data" / "feature_context_join.lock"


class RunLock:
    def __init__(self, path: Path) -> None:
        self.lock = JsonRunLock(path, "feature_context_join", stale_seconds=3600)
...
def run_cycle(symbols: list[str] | None, limit_windows: int | None, dry_run: bool) -> dict[str, Any]:
    db = SessionLocal()
    run = CollectorRun(
        collector_name="feature_context_15m_1h",
        status="RUNNING",
        started_at=utcnow(),
        finished_at=None,
        target="15m_1h",
    ...
    db.refresh(run)
    try:
        service = FeatureContextJoinService(db)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build MarketLab 15m trigger plus 1h context join.")
    ...
    while not stop_requested:
        ...
        if not lock.acquire():
            print(f"{utcnow().isoformat()} feature_context_15m_1h skipped: lock exists at {LOCK_PATH}")
        else:
            try:
