...
from app.services.outcome_tracker_15m import OutcomeTracker15mService  # noqa: E402
from app.services.utils import duration_seconds, json_safe, utcnow  # noqa: E402

LOCK_PATH = ROOT / "data" / "outcome_tracker_15m.lock"
LOCK_STALE_SECONDS = int(os.getenv("MARKETLAB_OUTCOME_LOCK_STALE_SECONDS", "3600"))
...
class RunLock:
    ...
    def _remove_stale_lock(self) -> bool:
        ...
        try:
            ...
            pass
        print(f"{utcnow().isoformat()} outcome_tracker_15m removed stale lock at {self.path}: {reason}")
        return True
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build read-only 15m candidate forward outcomes.")
    ...
    while not stop_requested:
        ...
        if not lock.acquire():
            print(f"{utcnow().isoformat()} outcome_tracker_15m skipped: lock exists at {LOCK_PATH}")
        else:
            try:
