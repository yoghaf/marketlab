...
def run_cycle(symbols: list[str] | None, limit_windows: int | None, dry_run: bool) -> dict[str, Any]:
    db = SessionLocal()
    run = CollectorRun(
        collector_name="psychology_labeler_15m",
        status="RUNNING",
        started_at=utcnow(),
        finished_at=None,
        target="15m",
    ...
    db.refresh(run)
    try:
        service = PsychologyLabeler15mService(db)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build read-only 15m psychology context labels.")
    ...
    while not stop_requested:
        ...
        if not lock.acquire():
            print(f"{utcnow().isoformat()} psychology_labeler_15m skipped: lock exists at {LOCK_PATH}")
        else:
            try:
