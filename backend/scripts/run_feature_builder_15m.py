...
from app.services.feature_builder_15m import FeatureBuilder15mService  # noqa: E402
...
LOCK_PATH = ROOT / "data" / "feature_builder_15m.lock"


class RunLock:
    def __init__(self, path: Path) -> None:
        self.lock = JsonRunLock(path, "feature_builder_15m", stale_seconds=3600)
...
def run_cycle(symbols: list[str] | None, limit_windows: int | None, dry_run: bool) -> dict[str, Any]:
    db = SessionLocal()
    run = CollectorRun(
        collector_name="feature_builder_15m",
        status="RUNNING",
        started_at=utcnow(),
        finished_at=None,
        target="15m",
    ...
    db.refresh(run)
    try:
        service = FeatureBuilder15mService(db)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build MarketLab 15m features from local aligned data.")
    ...
    while not stop_requested:
        ...
        if not lock.acquire():
            print(f"{utcnow().isoformat()} feature_builder_15m skipped: lock exists at {LOCK_PATH}")
        else:
            try:
