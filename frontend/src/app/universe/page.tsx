  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <MetricCard label="Active" value={data.active_universe_count ?? data.universe_count ?? data.count} />
        <MetricCard label="Full Active" value={data.full_active_count ?? 0} />
        <MetricCard label="Signal Eligible" value={data.signal_eligible_count ?? 0} />
        <MetricCard label="Shown" value={items.length} />
      </section>
      <FilterBar>
        ...
        <div className="table-wrap max-h-[70vh]">
          <table>
            ...
            <tbody>
              ...
              {!items.length && (
                <tr>
                  <td colSpan={9}><EmptyState title="Universe kosong" detail="Tidak ada symbol yang cocok dengan filter." /></td>
                </tr>
              )}
            </tbody>
          </table>
